...
test("plans a private saved policy when the directory is absent", async () => {
  const directory = await realpath(await mkdtemp(join(tmpdir(), "saved-policies-empty-")));
...
test("preserves and rechecks the exact selected existing policy snapshot", async () => {
  const directory = await realpath(await mkdtemp(join(tmpdir(), "saved-policies-existing-")));
  temporaryDirectories.push(directory);
...
test("keeps per-destination saved policy bytes independent", async () => {
  const directory = await realpath(await mkdtemp(join(tmpdir(), "saved-policies-destinations-")));
  temporaryDirectories.push(directory);
...
async function privateSavedPolicyDirectory(
  ...
  const directory = await realpath(await mkdtemp(join(tmpdir(), "saved-policies-test-")));
  temporaryDirectories.push(directory);
